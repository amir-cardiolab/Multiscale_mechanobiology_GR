...
#initialize vector for f_C
C_source_numpy = np.zeros((local_dim))
...
growth_rate.vector().set_local(C_source_numpy * growth_mult)
...
def ODEfunc(t,y,fC,k):
    ...
    tr = ctrace.vector()[i]
    s = Stressfunc(tr)
    dydt[A] = k * (-(y[A] * y[B]) + fs - (kd * y[A]))
    dydt[B] = k * (-(y[A] * y[B]) + fs - (kd * y[B]))
    dydt[C] = k * ((fC * s) + (y[A] * y[B]) - (kdc * y[C]))
    return dydt
