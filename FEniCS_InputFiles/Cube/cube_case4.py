...
sigmoid_I1 = Expression('1.0 + a/(1.0+exp(-(I1-s0)/b))', I1=Function(Vs_cg), a=Constant(10.0), s0=Constant(0.3), b=Constant(0.05), degree=1)
q_A = Constant(0.25)
q_B = Constant(0.25)
q_C = Expression('s * exp(-pow((abs(x[0])+abs(x[1])),2)/(0.3))', s=sigmoid_I1, degree=1)
h = Constant(dt)
kd = Constant(0.25)
kdC = Constant(1.0)
D = Constant(1e-20)
...
Fc  = (((u_1 - u_n1) / h) * v_1) * dx + (D * dot(grad(u_1),grad(v_1))) * dx - (k * q_A * v_1) * dx + (k * u_1 * u_2 * v_1) * dx + (k * kd * u_1 * v_1) * dx\
    + (((u_2 - u_n2) / h) * v_2) * dx + (D * dot(grad(u_2),grad(v_2))) * dx - (k * q_B * v_2) * dx + (k * u_1 * u_2 * v_2) * dx + (k * kd * u_2 * v_2) * dx\
    + (((u_3 - u_n3) / h) * v_3) * dx + (D * dot(grad(u_3),grad(v_3))) * dx - (k * q_C * v_3) * dx - (k * u_1 * u_2 * v_3) * dx + (k * kdC * u_3 * v_3) * dx
...
def UpdateStressVals():
    ...
    sigmoid_I1.I1 = CauchyI1

def UpdateSpeciesVals():
    C_source.interpolate(q_C)
...
xdmf_var_fields = {"displacement":ub, "Lagrange Strain":LagrangeS, "Cauchy Stress":CauchyS, "Pressure":pressure,\
                    ...
                    "[A]":A_sp, "[B]":B_sp, "[C]": C_sp, "C source":C_source,\
