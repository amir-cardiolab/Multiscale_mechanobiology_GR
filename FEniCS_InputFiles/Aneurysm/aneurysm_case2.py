...
f0 = Function(Vv)
sigmoid_I1 = Expression('1.0 + 1.0/(1.0+exp(-(I1-s0)/b))', I1=Function(Vs_cg), s0=Constant(0.2), b=Constant(0.05), degree=1)
f_TGFB = Expression('s*0.5*exp(-abs(pow(x[2],3.0))/3.0)',s=sigmoid_I1,degree=3)
TGFB = Species()
MAPK = Species()
Smad = Species()
MMP = Species()
